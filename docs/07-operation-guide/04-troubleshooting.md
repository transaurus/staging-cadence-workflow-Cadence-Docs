...
## API high latency, timeout, Task dispatching slowness Or Too many operations onto DB and timeouts
* If it happens after you attempted to truncate tables in order to reuse the same database/keyspace for a new cluster, it's possible that the data is not deleted completely. You should make sure to shut down Cadence when truncating, and make sure the database is cleaned. Alternatively, use a different keyspace/database is a safer way.
